...
if __name__ == "__main__":
    # 初始化（只需一次）
    load_bvh_file('walk.bvh')
    
    print("=" * 80)
    print("函数使用示例")
    print("=" * 80)
    
   # 在示例部分（if __name__ == "__main__"），修改所有输出：

    # 示例 1
    print("\n示例 1: 获取第 0 帧 Hips 的位置（相对于 hips）")
    hips_pos = get_joint_position(0, 'Hips')
    print(f"Hips 位置: [{hips_pos[0]:.6f}, {hips_pos[1]:.6f}, {hips_pos[2]:.6f}]")

    # 示例 2
    print("\n示例 2: 获取第 10 帧 LeftHand 的位置（相对于 hips，单位：米）")
    try:
        left_hand_pos = get_joint_position(10, 'LeftHand', unit='m')
        print(f"LeftHand 位置: [{left_hand_pos[0]:.6f}, {left_hand_pos[1]:.6f}, {left_hand_pos[2]:.6f}]")
    except ValueError as e:
        print(f"错误: {e}")

    # 示例 3
    print("\n示例 3: 获取第 5 帧 Neck 的世界坐标位置")
    try:
        neck_world_pos = get_joint_position(5, 'Neck', relative_to_hips=False)
        print(f"Neck 世界坐标: [{neck_world_pos[0]:.6f}, {neck_world_pos[1]:.6f}, {neck_world_pos[2]:.6f}]")
    except ValueError as e:
        print(f"错误: {e}")

    # 示例 4
    print("\n示例 4: 获取第 0 帧所有关节的位置（相对于 hips，单位：米）")
    all_positions = get_all_joints(0, unit='m')
    print(f"共 {len(all_positions)} 个关节")
    print("\n前 10 个关节:")
    for i, (joint_name, pos) in enumerate(list(all_positions.items())[:10]):
        print(f"  {joint_name:20s}: [{pos[0]:12.6f}, {pos[1]:12.6f}, {pos[2]:12.6f}]")

    # 示例 5
    print("\n示例 5: 获取八叉树系统需要的 6 个关键点（第 0 帧）")
    keypoint_names = {
        'hips': 'Hips',
        'left_wrist': 'LeftHand',
    ...
    keypoint_data = {}
    for kp_name, bvh_name in keypoint_names.items():
        try:
            pos = get_joint_position(0, bvh_name, unit='m')
            keypoint_data[kp_name] = pos.tolist()
            print(f"  {kp_name:15s}: [{pos[0]:12.6f}, {pos[1]:12.6f}, {pos[2]:12.6f}] 米")
        except ValueError:
            print(f"  {kp_name:15s}: [未找到关节 '{bvh_name}']")

    # 生成可用于 demo.py 的格式
    if len(keypoint_data) == 6:
        print("\n可用于 demo.py 的格式:")
        print("{")
        for kp_name in ['hips', 'left_wrist', 'right_wrist', 'neck', 'left_ankle', 'right_ankle']:
            if kp_name in keypoint_data:
                pos = keypoint_data[kp_name]
                print(f'    "{kp_name}": [{pos[0]:.6f}, {pos[1]:.6f}, {pos[2]:.6f}],')
        print("}")