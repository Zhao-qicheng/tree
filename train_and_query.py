...
def print_combination_indices(combination_indices: list[int], prefix: str = "层"):
    """打印每层的组合索引"""
    ...
    try:
        ...
        for keypoints, sample_name, combination_indices in train_samples:
            ...
            if len(query_combination_indices) > 0 and len(combination_indices) > 0:
                # 计算路径相似度（暂时用简单的匹配）
                if query_combination_indices == combination_indices:
                    distance = 0.0
                else:
                    # 计算路径差异（暂时用简单的计数）
                    matches = sum(1 for q, s in zip(query_combination_indices, combination_indices) if q == s)
                    distance = len(query_combination_indices) - matches
